...
public partial class ReceiveFromSourceSelector
{
    public ReceiveFromSourceSelector()
    {
        InitializeComponent();
    }

    private void RadioButtonSpecificSource_CheckedChanged(object sender, EventArgs e) => 
        TextBoxUdpSourceIP.Enabled = RadioButtonSpecificSource.Checked;

    public string ConnectionString
    {
        get => RadioButtonAnySource.Checked ? "" : $"; receiveFrom={TextBoxUdpSourceIP.Text}";
        set
        {
            Dictionary<string, string> connectionData = value.ParseKeyValuePairs();

            if (connectionData.TryGetValue("receiveFrom", out string udpSourceIP))
            {
                RadioButtonSpecificSource.Checked = true;
                Forms.PMUConnectionTester.AssignHostIP(TextBoxUdpSourceIP, udpSourceIP);
            }
            else
            {
